...
export default function VideoUploader({ addedVideos, onChange, maxVideos = 5 }: {
    addedVideos: string[],
    maxVideos: number,
    ...
    const uploadSingleVideo = async (file: any) => {
    ...
    const validateFiles = (files: any) => {
        const MAX_PHOTOS = maxVideos;
        const MAX_FILE_SIZE = 10 * 1024 * 1024;

        if (files.length + addedVideos.length > MAX_PHOTOS) {
    ...
    const handleImageUpload = async (e: any) => {
        ...
        try {
            ...
            const urls = await Promise.all(compressedFiles.map(uploadSingleVideo));

            toast({
                title: "Photos uploaded successfully",
                description: "You can now add more photos or submit the form",
            })
            // @ts-ignore
    ...
    async function removePhoto(ev: any, filename: any) {
        ...
        onChange([...addedVideos.filter(video => video !== filename)]);
    }

    function selectAsMainPhoto(ev: any, filename: any) {
        ev.preventDefault();
        onChange([filename, ...addedVideos.filter(video => video !== filename)]);
    }
